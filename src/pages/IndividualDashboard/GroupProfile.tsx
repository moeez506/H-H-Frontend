...
const GroupProfile = () => {
  ...
  const representativeData = memberData.filter(
    (member: any) => member.isGroupRespresentative
  );
  console.log(
    "🚀 ~ file: GroupProfile.tsx:26 ~ GroupProfile ~ representativeData:",
    representativeData
  );

  return (
    <div className="ml-28 mobile:ml-0 tabletOnly:ml-14 tabletOnly:w-max tabletScreen:ml-0">
      <h1 className="text-4xl font-semibold text-orange">{associationName}</h1>
      <br></br>
      <div className="shadow-inset shadow-2xl rounded-xl w-full">
        ...
        <h1 className="text-xl font-semibold p-4">Eligibility For Programs:</h1>
        <div className="flex flex-row mobile:flex-wrap">
          <div className="w-full sm:w-1/2 lg:w-1/3 px-4 mb-8 mobile:flex mobile:justify-between mobile:mb-0">
            <h2 className="text-orange text-xl font-semibold mb-2">
      ...
      {representativeData.map((data: any, index: any) => {
        return (
          <div
            key={data._id}
            className="shadow-inset shadow-xl rounded-xl mb-4"
          >
            <div className="flex justify-between p-2">
              ...
              <NavLink
                to={`/individual-Detail/${data._id as string}`}
                className="text-orange text-base pr-4 hover:underline"
              >
                View Detail
              </NavLink>
            </div>
